...
For Days 1 to 2 we built Reading 1 (the HTTP platform). It worked: gateway, queue, worker pool, hello smoke test, manifest scanner, all running on local Postgres with end-to-end idempotency proven by curl.
...
2. **The grading axis on trigger.** "Skill description 能否被 Claude 精準 trigger" by **Claude**, where Claude is the model, not our router. Reading 1 has no Claude-in-the-loop on routing; Reading 2 puts the model on the routing critical path, where the eval can grade it.
